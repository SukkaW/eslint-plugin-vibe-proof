...
runTest({
  ...
  invalid: [
    {
      code: 'const method = "find"; if (arr[method](fn)) {}',
      output: 'const method = "find"; if (arr.some(fn)) {}',
      errors: [{ messageId: 'preferArraySome' }]
    },
    {
      code: 'const size = "length"; if (arr["filter"](fn)[size]) {}',
      output: 'const size = "length"; if (arr.some(fn)) {}',
      errors: [{ messageId: 'preferArraySome' }]
    },
