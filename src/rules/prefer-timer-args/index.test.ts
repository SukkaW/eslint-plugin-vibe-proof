...
runTest({
  ...
  valid: [
    ...
    // Shadowed timer globals are unrelated functions.
    'function run(setTimeout) { setTimeout(() => fn(value), 100); }',
    'const window = { setTimeout }; window.setTimeout(() => fn(value), 100);',
    'function run(globalThis) { globalThis.setInterval(() => fn(value), 100); }',
    ...
    // Moving side effects out of the callback changes when they run.
    'setTimeout(() => fn(i++), 100)',
    'setTimeout(() => fn(delete obj.value), 100)',
    ['setTimeout(() => fn(`$', '{getData()}`), 100)'].join('')
  ],

  invalid: [
    {
      code: 'globalThis["setTimeout"](() => fn(value), 100)',
      output: 'globalThis["setTimeout"](fn, 100, value)',
      errors: [{ messageId: 'preferArgs' }]
    },
    {
      code: 'setTimeout(fn["bind"](void 0, value), 100)',
      output: 'setTimeout(fn, 100, value)',
      errors: [{ messageId: 'preferArgs' }]
    },
